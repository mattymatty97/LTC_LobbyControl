...
namespace LobbyControl.Patches
{
    ...
    internal class ItemClippingPatch
    {
		...
		public class PlaceableObjectsSurfacePatch
		{
			...
			public static bool itemPlacementPositionPatch(PlaceableObjectsSurface __instance, ref Vector3 __result, Transform gameplayCamera, GrabbableObject heldObject)
			{
				RaycastHit val = default(RaycastHit);
				if (Physics.Raycast(gameplayCamera.position, gameplayCamera.forward, out val, 7f, StartOfRound.Instance.collidersAndRoomMask, (QueryTriggerInteraction)1))
				{
					...
					Vector3 hitPoint = __instance.placeableBounds.ClosestPoint(val.point);
		...
		public class StartOfRoundPatch
		{
			...
			public static void AwakePatch(StartOfRound __instance)
			{
				foreach (Item items in __instance.allItemsList.itemsList)
				{
					...
					if (value.Count > 1)
					{
						items.restingRotation.Set(value[1], value[2], value[3]);
					}
				}
			}
